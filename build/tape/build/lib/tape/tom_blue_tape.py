...
class MyBlueTapeFollower(Node):
    def __init__(self):
        ...
        self.linear_velocity_gain = 0.135 #will stay constant but tune here
        self.proportional_gain = 1.4 #will be used to steer towards centroid of black tape
        self.integral_gain =     0.0 #0.0000001
        self.derivative_gain =   0.4 # 0.000001

        self.dt = 1/28 # camera publishes at 28 fps, this is the dt
    ...
    def listener_callback(self, data):
        ...
        if MR['m00'] > 0 or ML['m00'] > 0: #'m00' is all of the points that are in the mask (within the gray range)
            ...
            left_pixels = MR['m00']
            raw_error = (left_pixels - right_pixels)
            max_possible_error = max(MR['m00'], ML['m00'])  # use the larger region moment as normalization reference
            ...
            # dt = 1/28   # camera publishes at 28 fps, this is the dt
            p = self.proportional_gain * error

            self.integral += error * self.dt
            i = self.integral_gain * self.integral
            
            d = self.derivative_gain * ((self.previous_error -  error) / self.dt)
            ...
            angular_velocity = np.clip(angular_velocity, -0.4, 0.15) # was -0.3,0.3
            ...
            if error > 0:
                ...
                #     angular_velocity = np.clip(angular_velocity, -0.3, 0.3) # was -0.3,0.3
